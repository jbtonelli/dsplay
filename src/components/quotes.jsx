...
function QuotesContent() {
    ...
    useEffect(() => {
        ...
        if (!quotes || (new Date().getTime() - quotes.timestamp > 1000 * 60 * 50)) {
            (async () => {
                try {
                    const res = await axios(`${CORS_PROXY}https://free.currencyconverterapi.com/api/v6/convert?q=${pair1},${pair2}&compact=ultra&apiKey=${key}`);
